...
function unpackOnce(src, from = 0) {
  ...
  try {
    // Evaluate the inner function-call expression (safe: it only builds a string).
    // eslint-disable-next-line no-eval
    const result = eval('(' + block.inner + ')');
    if (typeof result === 'string') return { code: result, end: block.end };
    return null;
  } catch (e) {
    return null;
  }
